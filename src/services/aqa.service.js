...
const processSubmission = async (submissionId) => {
  ...
  // 2. Update submission with AQA results
  ...
  // 3. Trigger financial action based on decision
  ...
  switch (evaluation.decision) {
    ...
    case 'REFUND':
      await stripeService.triggerRefund({
        ...
        milestoneId: milestone.id,
        amount: milestone.amount,
        reason: `Refund due to failed AQA review for milestone: ${milestone.title}`,
      });
  ...
  // 5. Recalculate PFI for the freelancer
